...
def read_file(file_location:str, sep:str|None=None):
    ...
    if file_type in supported_file_types:
        ...
        match file_type:
            case "csv":
                df = pd.read_csv(file_location, sep=sep) # type: ignore
                return df 
            case "json":
...
def main():            
    ...
    if df is None:
        print(f">>>[Error Info]:File type unsupported. No DATA loaded")
...
########################### start of page content ###############################################
    ...
    # Display the resized image
        ...
        df.rename(rename_columns_dict,inplace=True) #type: ignore
        ...
        fan_Age_count = df.groupby('Age').agg( # type: ignore
            starwars_fan=('starwars_fan', 'count'),
            star_trek_fan=('star_trek_fan', 'count'), 
            Expanded_universe_fan=("Expanded_universe_fan","count"),
            Count=('Location', 'size')
    ...
    # Count occurrences of each location
        counts = df.groupby("Location").size().reset_index() #type: ignore
        ...
        fan_location_count = df.groupby('Location').agg( #type: ignore
            starwars_fan=('starwars_fan', 'count'),
            star_trek_fan=('star_trek_fan', 'count'),
            Expanded_universe_fan=("Expanded_universe_fan","count"),
            Count=('Location', 'size')
        ...
        bar_plot_plotly(counts_sorted, x='Location', y='count', title='Fans Location',title_text="Fans Location")
        bar_plot_plotly(fan_location_count, x='Location', y='starwars_fan', title='Star Wars Fans Location',title_text="Star Wars Fans Location")
        bar_plot_plotly(fan_location_count, x='Location', y='star_trek_fan', title='Star Trek Fans Location',title_text="Star Trek Fans Location")
        bar_plot_plotly(fan_location_count, x='Location', y='Expanded_universe_fan', title='Expanded Universe Fans Location',title_text="Expanded Universe Fans Location")
